...
class TetrisScene {
    ...
    func showScores(_ scores: Int) {
//        let isContains = self.scene.rootNode.animationKeys.contains{ (item) -> Bool in
//            return item == "scores"
//        }
//        if(isContains == true){
//            self.scene.rootNode.removeAnimation(forKey: "scores")
//        }
        let x = Float(config.width / 2 - 2)
        let y = Float(-2.0)
        let node = createNode(text("分数：\(scores)"), translate(x, y).scale(0.003), TetrisScene.scoresColor)
        //let y = node.position.y
        animateForScores(node, "position.y", from: y, to: y + cell * 4, during: 1000)
        animateForScores(node, "opacity", from: 1, to: 0, during: 1000)
        scoresNode = node
        self.scene.rootNode.addChildNode(scoresNode)
    }
	
    func updateScores(_ scores: Int) {
        self.scoresNode.removeFromParentNode()
        //self.scene.rootNode.removeAnimation(forKey: "scores")
    ...
    private func animateForScores(_ node: SCNNode, _ path: String, from: Any, to: Any, during: CFTimeInterval) {
        ...
        node.addAnimation(animation, forKey: "scores")
    }
